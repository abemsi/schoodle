...
const queryFunk = (sqlText, values) => {
  ...
  .then((res) => {
    ...
    console.log('Query func function:', err)
  })
};
...
const addChoice =  function(option_id, attendeeId, pollId) { 
  console.log('option_id, attendeeId, pollId', option_id, attendeeId, pollId)
  return queryFunk(`
...
const getAllPollInformation = function(link) {
  ...
  return queryFunk( 
    `SELECT options.id AS id, to_char("date", 'DD MON YYYY') as date, poll_id, title, description, location, link, organizer_id
    FROM options
   ...
   return queryFunk(`SELECT users.name AS name, users.email AS email, users.id AS attendeeID, option_id AS choice
    FROM choices
    JOIN polls ON polls.id = poll_id
    ...
    WHERE link = $1`, [link]).then(res => {
      ...
      const results = res.rows.reduce(function(availabilities, attendeeChoice) {
        console.log('line 152 db.js', attendeeChoice);
